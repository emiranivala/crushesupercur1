...
def upload_catbox(file_path):
    ...
    # Always use crushe.jpg from static folder
    upload_path = 'static/crushe.jpg'
    files = {
        'reqtype': (None, 'fileupload'),
        'userhash': (None, ''),
        'fileToUpload': (upload_path, open(upload_path, 'rb'), 'image/jpeg')
    }
...
async def add_thumb(query):
    # Always use crushe.jpg as thumbnail
    file_path = 'static/crushe.jpg'
    url = upload_catbox(file_path)
    await db.set_thumbnail(query.from_user.id, url)
    await query.message.reply_text("✅️ Your thumbnail has been set to crushe.jpg.")
...
async def settings(_, message):
    ...
    if lol == 1:
        return
    await message.reply_photo(photo="static/crushe.jpg", caption=script.SETTINGS_TXT, reply_markup=buttons1)
